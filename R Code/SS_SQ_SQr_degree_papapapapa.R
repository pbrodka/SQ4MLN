...
path <- "D:/SS4MLN/papapapapa/"



#full/compleete network
fullnet <- read_ml("d:/SS4MLN/FullNet/papapapapaNoLoops.mpx", name="papapapapa", sep=',', aligned=FALSE)
#fullnet <- read_ml("d:/SS4MLN/FullNet/papapapapaTransportation_multiplex_4NoNature.edges", name="papapapapa", sep=',', aligned=FALSE)
...
for(i in 1:length(file.names)){
  ...
  top_actors <- actors_ml(fullnet)[order(-neig,actors_ml(fullnet))]
...
#single stage seeding
  ...
  #results <- data.frame(network_name=factor(), network=factor(), pp=numeric(), network_id=numeric(), seed_count=numeric(), no_of_seeds=numeric(), no_of_activated=numeric(), no_of_iterations=numeric())
    ...
    save_path <- "D:/SS4MLN/Results/results_papapapapa_degree_ss.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_papapapapa_degree_sq.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_papapapapa_degree_sqr.txt"

...
fullnet <- read_ml("d:/SS4MLN/FullNet/CKM-Physicians-Innovation_4NoNature.edges", name="CKM", sep=',', aligned=FALSE)
...
#single stage seeding
  ...
  #results <- data.frame(network_name=factor(), network=factor(), pp=numeric(), network_id=numeric(), seed_count=numeric(), no_of_seeds=numeric(), no_of_activated=numeric(), no_of_iterations=numeric())
    ...
    save_path <- "D:/SS4MLN/Results/results_ckm_degree_ss.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_ckm_degree_sq.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_ckm_degree_sqr.txt"

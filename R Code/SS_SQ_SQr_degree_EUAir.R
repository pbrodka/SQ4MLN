...
fullnet <- read_ml("d:/SS4MLN/FullNet/EUAirTransportation_multiplex_4NoNature.edges", name="EUAir", sep=',', aligned=FALSE)
...
#single stage seeding
  ...
  #results <- data.frame(network_name=factor(), network=factor(), pp=numeric(), network_id=numeric(), seed_count=numeric(), no_of_seeds=numeric(), no_of_activated=numeric(), no_of_iterations=numeric())
    ...
    save_path <- "D:/SS4MLN/Results/results_EUAir_degree_ss.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_EUAir_degree_sq.txt"
...
for (seed_count in seed_count_v) {
  ...
  save_path <- "D:/SS4MLN/Results/results_EUAir_degree_sqr.txt"
